...
impl VfsNodeOps for RootDirectory {
    ...
    fn rename(&self, old_path: &str, new_path: &str) -> VfsResult {
        ...
        if let Some((mount_fs, old_rest)) = self.find_best_mount(&old_normalized) {
            if let Some((_, new_rest)) = self.find_best_mount(&new_normalized) {
                if old_rest.is_empty() || new_rest.is_empty() {
                    Err(axfs_vfs::VfsError::PermissionDenied)
                } else {
                    mount_fs.root_dir().rename(old_rest, new_rest)
                }
            } else {
                self.main_fs.root_dir().rename(old_rest, &new_normalized)
            }
        } else {
            self.main_fs.root_dir().rename(&old_normalized, &new_normalized)
        }
    }
