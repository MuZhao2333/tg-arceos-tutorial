...
impl DirNode {
    ...
    /// Renames a child node from `old_name` to `new_name` in this directory.
    ///
    /// Both names must be in the same directory. Cross-directory rename is not supported.
    pub fn rename_node(&self, old_name: &str, new_name: &str) -> VfsResult {
        let mut children = self.children.write();

        // Source must exist
        if !children.contains_key(old_name) {
            log::debug!("rename_node: old_name '{}' not found", old_name);
            return Err(VfsError::NotFound);
        }

        // Destination must not already exist
        if children.contains_key(new_name) {
            log::debug!("rename_node: new_name '{}' already exists", new_name);
            return Err(VfsError::AlreadyExists);
        }

        log::debug!("rename_node: renaming '{}' -> '{}'", old_name, new_name);
        // Now safe to remove and re-insert
...
impl VfsNodeOps for DirNode {
    ...
    fn rename(&self, src_path: &str, dst_path: &str) -> VfsResult {
        log::info!("ramfs rename ENTRY: src={} dst={}", src_path, dst_path);
        ...
        if src_rest.is_none() && dst_rest.is_none() {
            self.rename_node(src_name, dst_name)
        } else {
            let children = self.children.read();
            let subdir_ref = children.get(src_name).ok_or(VfsError::NotFound)?.clone();
            drop(children);
            let subdir = subdir_ref.as_any().downcast_ref::<DirNode>()
                .ok_or(VfsError::NotADirectory)?;
            let inner_src = src_rest.unwrap();
            let inner_dst = dst_rest.unwrap_or(dst_name);
            VfsNodeOps::rename(subdir, inner_src, inner_dst)
        }
    }
